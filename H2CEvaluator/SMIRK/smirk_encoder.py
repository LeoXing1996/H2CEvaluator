...
class ExpressionEncoder(nn.Module):
    ...
    def forward(self, img):
        ...
        outputs["eyelid_params"] = torch.clamp(
            parameters[..., self.n_exp : self.n_exp + 2], 0, 1
        )
        outputs["jaw_params"] = torch.cat(
            [
                F.relu(parameters[..., self.n_exp + 2].unsqueeze(-1)),
                torch.clamp(
                    parameters[..., self.n_exp + 3 : self.n_exp + 5], -0.2, 0.2
                ),
            ],
            dim=-1,
        )
